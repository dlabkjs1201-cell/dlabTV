...
class Replayer:
    def __init__(self, root):
        ...
        self.bar_canvas.bind("<Button-1>", self.click_bar)
        self.bar_canvas.bind("<B1-Motion>", self.click_bar)
    ...
    def toggle_settings(self):
        if self.settings_visible:
            ...
            self.settings_popup.geometry(f"200x180+{x}+{y}")
            self.settings_popup.configure(bg="#f0f0f0", bd=2, relief="ridge")
            tk.Label(self.settings_popup, text="🔊 볼륨", bg="#f0f0f0", font=("맑은 고딕", 9, "bold")).pack(pady=(20, 5))
            self.vol_canvas = tk.Canvas(self.settings_popup, bg="white", height=14, highlightthickness=1)
            self.vol_canvas.pack(padx=20, fill="x")
            self.vol_canvas.bind("<Button-1>", self.click_volume)
            self.vol_canvas.bind("<B1-Motion>", self.click_volume)
            self.root.after(10, self.draw_volume_bar)
            tk.Label(self.settings_popup, text="⏩ 재생 속도", bg="#f0f0f0", font=("맑은 고딕", 9, "bold")).pack(pady=(20, 5))
            sf = tk.Frame(self.settings_popup, bg="#f0f0f0")
            sf.pack(pady=5)
            for s in [0.5, 1.0, 1.5, 2.0]:
                btn = tk.Button(sf, text=f"{s}x", command=lambda v=s: self.set_speed(v), width=4)
                btn.pack(side="left", padx=2)
                if s == self.playback_speed:
                    btn.configure(bg="#0078d7", fg="white")
            self.settings_visible = True

    def click_volume(self, event):
        self.volume = max(0, min(event.x / self.vol_canvas.winfo_width(), 1)) * 100
        self.draw_volume_bar()

    def draw_volume_bar(self):
        if not self.settings_popup or not hasattr(self, 'vol_canvas'):
            return
        self.vol_canvas.delete("vol")
        self.vol_canvas.create_rectangle(0, 0, self.vol_canvas.winfo_width() * (self.volume / 100), 14,
                                         fill="#0078d7", outline="", tags="vol")

    def set_speed(self, speed):
        self.playback_speed = speed
        if self.cap:
            self.start_time = time.time()
            self.start_frame = self.cap.get(cv2.CAP_PROP_POS_FRAMES)
    ...
    def draw_progress_bar(self, percent):
        ...
        if w > 1:
            self.bar_canvas.create_rectangle(0, 0, w * percent, 14, fill="#0078d7", outline="", tags="prog")

    def click_bar(self, event):
        if not self.cap:
            return
        ...
        if self.is_playing:
            ...
            self.start_time = time.time()
            self.start_frame = target_f

            if self.audio_device and self.temp_audio_path:
                self.audio_stream = miniaudio.stream_file(self.temp_audio_path)
                try:
                    sr = miniaudio.get_file_info(self.temp_audio_path).sample_rate
                except:
                    sr = 44100
                skip_samples = int((target_f / self.fps) * sr)
                for _ in range(skip_samples // 1024):
                    next(self.audio_stream, None)
    ...
    def load_video(self, force_path=None):
        ...
        if self.extract_audio(path):
            # ⭐ [수정 핵심] buffersize_msec=20으로 지정하여 0.2초 딜레이 방지 및 UI 렉 제거
            try:
    ...
    def stop_video(self):
        ...
        dev_to_close = self.audio_device
        path_to_delete = self.temp_audio_path
        self.audio_device = None
        self.temp_audio_path = ""

        def async_clean(dev, path):
            if dev:
                try:
                    dev.stop()
                    dev.close()
        ...
        threading.Thread(target=async_clean, args=(dev_to_close, path_to_delete), daemon=True).start()
    ...
    def toggle_play(self, force_play=False):
        if not self.cap:
            return
        self.is_playing = force_play if force_play else not self.is_playing
        ...
        if self.is_playing:
            ...
            # ⭐ [수정 핵심] 재생 시 오디오가 엉뚱한 곳에서 튀어나오지 않게 무조건 현재 영상 위치로 리셋 정렬
            if self.audio_device and self.temp_audio_path:
                try:
                ...
                try:
                    ...
                    sr = 44100
                skip_samples = int((self.start_frame / self.fps) * sr)
                for _ in range(skip_samples // 1024):
                    next(self.audio_stream, None)
            ...
            # ⭐ [수정 핵심] 버퍼를 20ms로 줄였기 때문에 스레드 없이 바로 멈춰도 렉이 안 걸립니다! (0.2초 트레일링 완벽 해결)
    ...
    def video_worker(self):
        while self.is_playing and self.cap:
            if self.seek_target is not None:
                ...
                self.start_time = time.time()
                self.start_frame = self.seek_target
                ...
                if self.audio_device and self.temp_audio_path:
                    ...
                    try:
                        ...
                        sr = 44100
                    skip_samples = int((self.seek_target / self.fps) * sr)
                    for _ in range(skip_samples // 1024):
                        next(self.audio_stream, None)
                    self.audio_device.start(self.audio_stream)
            ...
            target_f = self.start_frame + int(elapsed * self.fps * self.playback_speed)
    ...
    def update_frame(self, once=False):
        if self.cap and (self.is_playing or once):
            ...
            if self.is_playing and not once:
    ...
    def match_ratio(self):
        ...
        if w > 0:
            self.root.geometry(f"{940}x{int(920 * (h / w)) + 180}")
